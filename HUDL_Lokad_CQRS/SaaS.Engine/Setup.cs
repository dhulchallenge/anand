...
namespace SaaS.Wires
{
    public sealed class Setup
    {
        ...
        public const string TableContainer = "RentalBooking";
        public Container Build()
        {
            ...
            providers.BuildTableStorage().DeleteTable("CarRentalMaster");
            var bookingMaster = new CloudTable<CarRentalMaster>(providers.BuildTableStorage(), "CarRentalMaster");
            

            bookingMaster.Insert(
            new CloudEntity<CarRentalMaster>
                {
                    PartitionKey = "Ford",
                    RowKey = "1",
                    Timestamp = DateTime.UtcNow,
                    Value = new CarRentalMaster { Availabitiy = 100 }
                }
                );
            bookingMaster.Insert(
               new CloudEntity<CarRentalMaster>
                {
                    ...
                    RowKey = "2",
                    Timestamp = DateTime.UtcNow,
                    Value = new CarRentalMaster { Availabitiy = 70 }
                }
                );
            bookingMaster.Insert(
             new CloudEntity<CarRentalMaster>
             {
                 ...
                 RowKey = "3",
                 Timestamp = DateTime.UtcNow,
                 Value = new CarRentalMaster { Availabitiy = 50 }
             }
              );
