        ...
        static void Main()
        {
            ...
            using (var cts = new CancellationTokenSource())
            {
                ...
                using (var engine = env.BuildEngine(cts.Token))
                {
                   ...
                   // env.SendToCommandRouter.Send(new CreateUser(new UserId(5),new SecurityId(5)));
                  //  env.SendToCommandRouter.Send(new CreateSecurityAggregate(new SecurityId(111)));
                    env.SendToCommandRouter.Send(new CreateBooking(new BookId(Guid.NewGuid().ToString()),new Rental{ ModelName = "Ford" , BookingFrom = DateTime.Now, BookingTo = DateTime.Now.AddDays(2)}) );
